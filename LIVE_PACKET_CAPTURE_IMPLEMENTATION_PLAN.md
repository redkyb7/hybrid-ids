...
[ OLD IMPLEMENTATION: Synthetic Flow Replay ]
  ...
  hybrid_engine.py (Offline Heuristics)
       ...
       ▼
  data/ids_logs.db ──> frontend/app.py (FastAPI UI)
...
[ NEW IMPLEMENTATION: Real-Time Streaming Ingestion & Live Virtual Testbed ]
  ...
  │ • FastAPI SOC Console (frontend/app.py on http://localhost:8000):                         │
...
- **Thread-Safe SQLite Persistence:** Uses `PRAGMA journal_mode=DELETE;` and synchronized thread locks to guarantee lock-free reads for the FastAPI dashboard on Docker-mounted volumes.
...
# 3. Launch the FastAPI Monitoring Console on host
python frontend/app.py
```

Open **`http://localhost:8000`** in your browser. You will observe:
...
├── frontend/
│   ├── app.py                    # FastAPI telemetry API and dashboard server
│   └── templates/index.html      # Browser-based real-time SOC dashboard
├── data/
